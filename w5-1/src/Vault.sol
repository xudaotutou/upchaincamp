...
contract Vault {
    ...
    function tokensReceived(address sender, uint amount)
        external
        returns (bool)
    {
    ...
    function depositWithAddress(address account, uint amount)
        public
    ...
    function collect() external {
        if (LYKT.balanceOf(address(this)) > 100e18) {
            LYKT.transfer(owner, LYKT.balanceOf(address(this)));
        }
    }
}
